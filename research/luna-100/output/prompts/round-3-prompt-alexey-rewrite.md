...
Check that the brief describes the assigned source before relying on it. If
it points to another article or lists facts absent from the supplied source,
rebuild the brief from that source. Preserve the discrepancy in the editorial
report. The original source outranks the brief. Preserve numerical
bounds and the meaning of time periods: six or more is not more than six,
and postings from the previous three months need not have taken three months
to collect. Do not make an untested component sound deployed.
...
2. Put the author or reader back into a sentence that hides a decision.
   ...
   An explanation in Alexey's voice should not call him the author or the
   researcher. Keep other people's work attributed to them, and preserve
   hypothetical examples rather than inventing personal project history.
...
6. Join unnecessarily separated thoughts when the cause and decision belong
   ...
   Do not invent a because or so relationship to join short sentences. Keep
   the original relationship, even when it is just two coexisting requirements.
