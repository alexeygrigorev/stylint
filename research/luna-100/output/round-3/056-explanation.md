...
The writing scripts use `plan.yaml` without needing the planning chat history. The chapter-based command is `uv run python -m chapter_based.execute mybook`, with `make generate-book` available through the Makefile. The repository also has a section-based implementation for longer books, but the chapter-based path makes the workflow easier to see.
