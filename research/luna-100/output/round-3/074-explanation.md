...
The layout can stay small. Use a package directory with `__init__.py` and a version file, then add the test directory and configuration files. I use Hatch with the hatchling build backend for packaging, alongside uv for tooling. A command-line package also needs a project script entry point so installation exposes the command.
