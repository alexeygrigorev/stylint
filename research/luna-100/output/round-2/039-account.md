...
`dirdotenv` loads environment variables when I enter a project directory while keeping ordinary `.env` files. `direnv` uses `.envrc`. The other tools I use already read `.env` files.
...
They're small tools, but each one fits a workflow I already have. That's enough reason for me to keep them around. I can change them when my workflow changes because I own the code. That flexibility was part of the reason to build them. I wanted tools that matched the details of my daily work.
