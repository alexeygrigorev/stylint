...
A book generator becomes easier to control when planning is separate from writing. The pipeline starts with an interactive outline in Streamlit, converts that outline into a typed YAML plan and uses the same plan for chapters, audio and publishing. The plan is the shared interface between stages.
...
For each chapter, the script builds progress markers. Completed chapters receive `[x]`, the current chapter gets an arrow and future chapters receive `[ ]`. Gemini receives the progress outline and the current chapter's bullet points, but not the full text of earlier chapters. Cohesion comes from the initial plan. Existing chapter files are skipped on reruns, and a `_ready` sentinel marks a completed book.
