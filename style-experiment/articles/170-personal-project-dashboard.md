...
I didn't need another productivity method. I needed one page that answered what's active, what's next, and what I touched this week.
...
So I reduced the unit to a single project record. Each project has a name, state, deadline, and owner. It also has a repository URL and one next action. Tasks stay in their original systems. The dashboard only knows the first step I've committed to.
...
My first version allowed states such as "waiting" and "paused" plus "exploring", "blocked", and "maintenance". They felt precise, but I couldn't decide where several projects belonged.
...
Active means I plan to touch it this week. A paused state means I don't plan to touch it, but I want to keep the context. Waiting means another person or event controls the next move. Done means the deliverable exists.
...
## Next Actions That Start With a Verb
...
When I complete an action, the script moves the project back to the top of the review list. It doesn't invent the following step. An empty field prompts me to spend two minutes planning.
...
I generate the snapshot page without JavaScript, and it weighs 18 KB. It opens easily on my phone when the main dashboard is unavailable.
