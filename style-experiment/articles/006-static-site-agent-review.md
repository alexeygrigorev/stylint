...
The page passed because it existed. It failed because I hadn't defined the review path. Since then, I use a five-part checklist before publishing any agent-edited site. The latest review took 16 minutes and caught four issues.

The site is a 14-page Astro project. It has no client-side JavaScript except one search form. That small scope makes review manageable, but it doesn't remove the need for a route-by-route pass.

In this post, I'll share:

- what the agent changed
- how I inspect routes
...
It also rewrote four sentences on the home page. I hadn't asked for that. The rewrite was defensible because the old sentence mentioned pricing, but it was outside the task boundary. This became the first review rule: compare the changed files with the request, then inspect the requested feature.
...
I start with routes because they define the public surface. The site map has 14 stable URLs, so I list the generated routes in the review note and compare them with the previous build. The pricing page should add one URL and change nothing else.
...
After the build, I open every changed route and one neighboring route. For the pricing change, I checked `/pricing`, `/`, `/services` and `/contact`. The neighboring pages showed that the footer now linked to pricing twice.
...
Then I run a link checker. The project uses `lychee`, a local link-checking tool. It found 11 internal links and 3 external links on the changed pages. One internal anchor failed, and one external documentation URL redirected to a new path.
...
I also check the social preview with the local development server. The pricing page inherited an image named `default.png`, which showed a code editor and didn't match the topic. I replaced it with a simple image containing the three plan names.
...
The desktop preview usually survives agent edits. Mobile is where details break. I test at 360 by 800 pixels and at 768 by 1,024 pixels, because those sizes cover the smallest common phone and a tablet-width breakpoint.
...
The final review now ends with a checklist in the pull request. It covers changed routes, copy claims, links, metadata, keyboard access and the two mobile widths. The reviewer can mark each item as checked or request a change.

I keep the checklist in `REVIEW.md`, next to the deployment instructions. The agent reads it when I ask for a pre-review, and I still run it myself. The tool can prepare the evidence; the person decides what ships.

On the latest pricing update, the agent prepared screenshots for four widths and a list of changed links. I found one mismatch between the plan JSON and the rendered annual price. The fix took three minutes.

The process is not fully automated because publication is a human decision. I want the agent to make the review easy. I don't want it to approve its own work.
