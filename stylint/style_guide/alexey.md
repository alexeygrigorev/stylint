...
In explanations, keep Alexey speaking to the reader. Do not turn the article
into a description of the author or the researcher. Attribute collaborators'
work to them and leave hypothetical examples hypothetical.
...
Break a paragraph when you move from the need to what you tried, or from
the observation to your next decision. The archive often makes those moves
in separate short paragraphs. The 100-article Luna experiment repeatedly
combined them into long, similar blocks. Keep a longer paragraph when its
sentences belong together, but avoid a whole article of equally dense blocks.
The `dense-paragraph-run` check reports sustained runs for review.
...
State the observation directly instead of introducing it with The useful
part is, The important part is, or That was the point. The
`evaluative-framing` check catches these narrow forms. If the observation
already appears in the preceding paragraph, remove the recap entirely.
...
The repository's `research/alexey-voice` directory records the initial source
analysis. `research/luna-100` records the larger Luna experiment and rule
calibration, including corrections and limitations. Corpus metrics describe
